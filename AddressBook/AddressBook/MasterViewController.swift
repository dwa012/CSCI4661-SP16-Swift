...
class MasterViewController: UITableViewController,
    ...
    func didSaveContact(controller: AddEditTableViewController) {
        ...
        do { // if no error, display new contact details
            try context.save()
            let sectionInfo =
                self.fetchedResultsController.sections![0] 
            if let row = (sectionInfo.objects as! [NSManagedObject]).indexOf(controller.contact!) {
        ...
        } catch let error1 as NSError { // check for error
            error = error1
